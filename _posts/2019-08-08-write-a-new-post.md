...
> 帖子的布局已默认设置为 `post` ,因此无需在前言块中添加post变量。
{: .prompt-tip }

### 日期时区

为了准确记录帖子的发布日期，您不仅应该在 `_config.yml`{: .filepath}中设置帖子的 `时区` ，而且还应该在其前言块的 `date` 变量中提供帖子的时区. 格式：`+/-TTTT`, 例如：`+0800`.
...
默认情况下，**T**able **o**f **C**ontents（TOC）显示在帖子的右侧面板上。如果要全局关闭它，请转到`_config.yml`{: .filepath}并将变量`toc`的值设置为`false`。如果你想关闭特定帖子的目录，请将以下内容添加到帖子的[Front Matter](https://jekyllrb.com/docs/front-matter/)中：
...
如果您想关闭特定帖子的评论，请将以下内容添加到帖子的 **Front Matter** 中：
...
然后，您可以像使用其他标记语言一样使用它：用 ```` ```mermaid ```` 和 ```` ``` ```` 围绕图形代码。
...
注意，[`img_path`](#image-path)也可以传递给预览图像，也就是说，当设置了它时，属性`path`只需要图像文件名。
