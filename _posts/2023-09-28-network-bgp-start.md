...
BGP路径属性分为公认属性和可选属性。公认属性是所有BGP协议实现必须都能正确识别的属性；可选属性是BGP协议实现可不支持的属性。公认属性又分为公认必遵和公认自决属性。**公认必遵**是在update报文中必须携带的属性；而**公认自决**并不要求必须包含在update报文中。可选属性分为可选传递和可选非传递。**可选传递**必须在报文接收到包含该属性的路由后，将属性传递给其他对等体；**可选非传递**即使在收到该属性的路由，可不支持也可不传递该属性给其他对等体。
